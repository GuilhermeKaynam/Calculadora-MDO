...
import BoxCalculatorForm from './BoxCalculatorForm';
import BoxResultDisplay from './BoxResultDisplay';
import BoxCalculationHistory from './BoxCalculationHistory';
import { useBoxContext } from '../context/BoxContext';
import { ClipboardList, Calculator } from 'lucide-react';

const BoxCalculator: React.FC = () => {
  const { addToHistory } = useBoxContext();
        ...
        {activeTab === 'calculator' ? (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-gray-50 p-4 sm:p-6 rounded-xl border border-gray-200">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Informações do Box</h2>
              <BoxCalculatorForm />
            </div>
            <div>
              <BoxResultDisplay onSave={addToHistory} />
            </div>
          </div>
        ) : (
          <BoxCalculationHistory />
        )}
      </div>
